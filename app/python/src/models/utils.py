...
def save_model(classifier, model_name, le):
    ...
    filenameSAV = model_name + "_" + str(now.date()) + "_" + str(now.hour) + "." + str(now.minute) + "." + str(
        now.second) + ".pkl"
    my_path = Path("..") / Path("..") / Path("..") / Path("..") / "storage" / "models" / "preprocess_characteristic" / model_name / filenameSAV
    ...
    filenameSAV = model_name + "_" + str(now.date()) + "_" + str(now.hour) + "." + str(now.minute) + "." + str(
        now.second) + ".sav"
    my_path = Path("..") / Path("..") / Path("..") / Path("..") / "storage"  / "models" / "training_models" / model_name / filenameSAV
...
def prediction(num_model, model_date, data_test):
    ...
    model_path = Path("..") / Path("..") / Path("..") / Path("..")  / "storage" / "models" / "training_models" / str(model_name[num_model]) / (filename + ".sav")
    with open(model_path, 'rb') as pickle_file:
        model = pickle.load(pickle_file)

    preprocess_path = Path("..") / Path("..") / Path("..") / Path("..") / "storage"  / "models" / "preprocess_characteristic" / str(model_name[num_model]) / (filename + ".pkl")
